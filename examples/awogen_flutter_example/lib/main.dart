...
import 'package:awogen_flutter_example/order.dart';
...
Future<void> main() async {
  ...
  final session = await account.createEmailSession(
      email: 'me25@appwrited.io', password: 'titititi');

  final realtime = Realtime(client);

  final subscription = realtime.subscribe([
    'databases.6462440df25577dd70a6.collections.6469580ad31ea625b715.documents'
  ]);
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void addOrderClient() async {
    Order order = Order(
        ...
        ip: AwIp("10.0.0.11"));
    Order order2 = await order.createObjectClientSdk();

    print(order2.documentId);
    print(order2.name);
    order2.name = "newname";
    order2.blabla = blabla_awEnum.gugu;
    order2 = await order2.updateObjectClientSdk();
    print(order2.toJSON());
    print(order2.documentId);
    print(order2.name);
    print(order2.amount);
